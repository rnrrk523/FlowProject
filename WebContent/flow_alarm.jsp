...
<%@page import="dto.ProjectAlarmListDto"%>
<%@page import="dao.ProjectAlarmDao"%>
<%@page import="java.util.Date"%>
<%@page import="dao.LiveAlarmDao"%>
<%@page import="dto.LiveAlarmDto"%>
...
<%-- <%
	...
	try {
		alarmList = lADao.getLiveAlarmSearchList(memberIdx, 'N', 'N', 'N', 'N', "");
	} catch(Exception e) {}
...
<head>
	...
	<link rel="stylesheet" href="css/Flow_alarm.css"/>
		...
		$(function() {
			...
			$(".checkmark").click(function() {
				if($(this).prev().is(':checked')){
					if($(".alarmSerachStandard:checked").length != 1) {
						$(this).css('background-image', 'url(https://team-0aaj7b.flow.team/flow-renewal/assets/images/icons/icon_check.png?v=468ce13b5d7bacf4e0668ba7bbd521a4532a5cae)');
						$(this).parent().css('color', '#000');
						$(this).prev().prop('checked', true);
					}
				}else{
					$(this).css('background-image', 'url(https://team-0aaj7b.flow.team/flow-renewal/assets/images/icons/icon_check_on.png?v=e8f6f2a4670b3cc32710588df4b57a6e7db79a1b)');
					$(this).parent().css('color', '#6449FC');
			...
			/*프로젝트 알림 검색하기*/
			$(".input-alarm").on('keydown', function(e) {
				if(e.key === 'Enter') {
					let str = $(this).val();
					let ctt = $('input[name="ctt"]').prop('checked') ? 'Y' : 'N';
					let writ = $('input[name="writ"]').prop('checked') ? 'Y' : 'N';
					$.ajax({
						type: 'post',
						url: 'ProjectAlarmSearchAjaxServlet',
						data: {
						...
						success: function(data) {
							...
							$(".alarm-items").remove();
							for(let i=0; i<=data.length-1; i++) {
								let newTr = '<div class="alarm-items">' +
												'<img src="'+data[i].prof+'" alt="user-prof"/>' +
												'<div class="alarm-prof"></div>' +
												'<div class="alarm-content-box">' +
													'<p class="alarm-title">'+data[i].title+'</p>' +
													'<p class="alarm-content">'+data[i].content+'</p>' +
													'<p class="alarm-date">'+data[i].alarmDate+'</p>' +
												'</div>' +
											'</div>';
								
								$(".alarm-item-box").append(newTr);
							}
						},
						error: function(r, s, e) {
							console.log(r.status);
	...
	<script>
		$(function() {
		    ...
		    $('.submit-project').click(function(){
		    	...
		    	let selectedOption5 = $('.file-view-auth-element').find(findstr);
		    	let dataCode5 = selectedOption5.data('code');
		    	let selectedOption6 = $('.file-down-auth-element').find(findstr);
		    	let dataCode6 = selectedOption6.data('code');
		    	if(publics==true) {
		    ...
		    $('#project-edit-set').click(function(){
	    		...
	    		$('.file-view-auth-element').find('option').filter(function() {
	    		    return $(this).data("code") == <%=pvdto.getFileViewGrant()%>;
	    		}).prop("selected", true);
	    		$('.file-down-auth-element').find('option').filter(function() {
	    		    return $(this).data("code") == <%=pvdto.getFileDownGrant()%>;
	    		}).prop("selected", true);
		    });
						...
						<dl class = "project-create-option2">
							...
							<dd class ="company-admin-option">
								...
								<dl class = "project-admin-more-option-area" style="display:">
									...
									<dd>
										<ul>
											<li class = "file-view-auth-element">
												<span>조회 권한</span>
												<div class = "select-authority-area">
													<select class = "authority-select-box">
														<option data-code = "0">전체</option>
											...
											<li class = "file-down-auth-element">
												<span>다운로드 권한</span>
												<div class = "select-authority-area">
													<select class = "authority-select-box">
														<option data-code = "0">전체</option>
		...
		<div class="mainarea">
			...
			<div class="content-wrap">
				<div class="search-box">
					...
					<input type="text" name="alarm_name" class="input-alarm" placeholder="검색"/>
					<div class="option-btn">옵션</div>
					<div class="option-box">
						<div><label for="1">내용<input type="checkbox" class="alarmSerachStandard" name="ctt" id="1" checked/><span class="checkmark"></span></label></div>
						<div><label for="2">작성자<input type="checkbox" class="alarmSerachStandard" name="writ" id="2" checked/><span class="checkmark"></span></label></div>
					</div>
				</div>
				<div class="alarm-item-box">
					<c:forEach var="dto" items="${projectAlarmList }">
						<div class="alarm-items">
							<img src="${dto.prof }" alt="user-prof"/>
							<div class="alarm-prof"></div>
							<div class="alarm-content-box">
								<p class="alarm-title">${dto.title }</p>
								<p class="alarm-content">${dto.content }</p>
								<p class="alarm-date">${dto.alarmDate.substring(0,10) }</p>
							</div>
						</div>
					</c:forEach>
				</div>

...
<head>
    ...
    <script>
    	$(function() {
    		/*내 게시물 검색옵션창 열기*/
    		$("#myboard-search-filterBtn").click(function() {
    			$("#myboard-search-filter-optionPopup").css('display', 'block');
    		})
    		/*내 게시물 검색옵션창 취소버튼 클릭*/
    		$("#myboard-search-fliter-optionPopup-cancleBtn").click(function() {
    			$("#myboard-search-projectName-input").val("");
    			$("#myboard-search-writerName-input").val("");
    			$("#searchDate-3month").prop('checked', true);
    			$("#searchTarget-all").prop('checked', true);
    			$("#myboard-search-filter-optionPopup").css("display", "none");
    		})
    		/*내 게시물 검색 input에서 엔터*/
    		$("#myboard-search-input").on("keydown", function(event) {
			    if (event.key === "Enter" || event.keyCode === 13) {
			    	let str = $("#myboard-search-input").val();
			    	$.ajax({
	    				type: 'post',
	    				url: 'myboardTextSearchAjaxServlet',
	    				data: {
	    					"memberIdx":${memberIdx},
	    					"str":str
	    				},
	    				success: function(data) {
	    					console.log(data);
	    					let showCnt = 0;
	    					$(".myboard-content-item").remove();
	    					$("#myboard-search-filter-optionPopup").css('display', 'none');
	    					for(let i=0; i<=data.length-1; i++) {
	    						if(data[i].boardCategory == "일정") {
	    							let filterState = $("#myboard-content-title1").text() == data[i].boardCategory ? "" : ($("#myboard-content-title1").text() == "전체" ? "" : ' style="display:none; "');
	    							let str = '<li class="myboard-content-item" data-boardIdx="'+data[i].boardIdx+'" '+filterState+'>'+
						    						'<div class="itemTypeWrap">'+
						    						'<div class="myboard-content-calendar-img"></div>'+
						    						'<span class="itemTypeSpan">일정</span>'+
						    					'</div>'+
						    					'<div class="itemBoardInfoWrap">'+
						    						'<div><p class="myboard-items-boardName">'+data[i].boardTitle+'</p></div>'+
						    						'<div><p class="myboard-items-projectName"><span class="project-icon"></span>'+data[i].projectName+'</p></div>'+
						    					'</div>'+
						    					...
						    					'</div>'+
						    				'</li>';
									$("#myboard-content-ul").append(str);
	    						}else if(data[i].boardCategory == "글") {
	    							let filterState = $("#myboard-content-title1").text() == data[i].boardCategory ? "" : ($("#myboard-content-title1").text() == "전체" ? "" : ' style="display:none; "');
	    							let str = '<li class="myboard-content-item" data-boardIdx="'+data[i].boardIdx+'" '+filterState+'>'+
					    								'<div class="itemTypeWrap">'+
					    							'<div class="myboard-content-text-img"></div>'+
	    							...
	    							let str = '<li class="myboard-content-item" data-boardIdx="'+data[i].boardIdx+'" '+filterState+'>'+
					    								'<div class="itemTypeWrap">'+
					    							'<div class="myboard-content-task-img"></div>'+
	    						...
	    						if($("#myboard-content-title1").text() == data[i].boardCategory) {
    								showCnt++;
    							}
	    					}
	    					if($("#myboard-content-title1").text() == "전체") {
	    						$("#myboard-content-title2").text(data.length);
	    					}else {
	    						$("#myboard-content-title2").text(showCnt);
	    					}
	    				},
	    				error: function(r, s, e) {
	    					console.log(r.status);
    		...
    		/*내 게시물 검색옵션창 검색버튼 클릭*/
    		$("#myboard-search-fliter-optionPopup-searchBtn").click(function() {
    			let projectSearchName = $("#myboard-search-projectName-input").val();
    			let writerSearchName = $("#myboard-search-writerName-input").val();
    			...
    			$.ajax({
    				type: 'post',
    				url: 'myboardOptionSearchAjaxServlet',
    				data: {
    					"memberIdx":${memberIdx},
    		...
    		$(document).on("click", function(event) {
    			// 클릭한 요소가 #myboard-show-filter-ul 내부에 있는지 확인
    			if(!$(event.target).closest("#myboard-show-filter-ul").length) {
    				// #myboard-show-filter-ul이 현재 보이는 상태인지 확인
    				if($("#myboard-show-filter-ul").is(":visible")) {
    					$("#myboard-show-filter-ul").hide(); // 메뉴 닫기
    				}
    			}
    		});
    		/*필터 클릭 시*/
    		$("#itemList-filterBtn").click(function() {
    			event.stopPropagation(); // 상위 document 클릭 이벤트 방지
    			 $("#myboard-show-filter-ul").toggle();
    		})
    		
    		/*item 클릭 시*/
    		$(document).on("click", ".myboard-content-item", function() {
    		...
    		$(document).ready(function() {
    			/*전체 클릭*/
        		$("#myboard-show-filter-menuBtn-all").click(function() {
        			$(".myboard-show-filter-menuBtn-on").addClass("myboard-show-filter-menuBtn-off");
        			$(".myboard-show-filter-menuBtn-on").removeClass("myboard-show-filter-menuBtn-on");
        			
        			$("#myboard-show-filter-menuBtn-all").removeClass("myboard-show-filter-menuBtn-off");
        			$("#myboard-show-filter-menuBtn-all").addClass("myboard-show-filter-menuBtn-on");
        			
        			$(".itemTypeSpan").each(function() {
        				$(this).parent().parent().css("display", "flex");
        			});
        			
        			$("#myboard-content-title1").text("전체");
        			let showItemsCnt = $(".myboard-content-item:visible").length;
        			$("#myboard-content-title2").text(showItemsCnt);
        		})
        		/*글 클릭*/
        		$("#myboard-show-filter-menuBtn-text").click(function() {
        			$(".myboard-show-filter-menuBtn-on").addClass("myboard-show-filter-menuBtn-off");
        			$(".myboard-show-filter-menuBtn-on").removeClass("myboard-show-filter-menuBtn-on");
        			
        			$("#myboard-show-filter-menuBtn-text").removeClass("myboard-show-filter-menuBtn-off");
        			$("#myboard-show-filter-menuBtn-text").addClass("myboard-show-filter-menuBtn-on");
        			...
        			$("#myboard-content-title1").text("글");
        			$("#myboard-content-title2").text(filterCnt);
        		})
        		/*업무 클릭*/
        		$("#myboard-show-filter-menuBtn-task").click(function() {
        			$(".myboard-show-filter-menuBtn-on").addClass("myboard-show-filter-menuBtn-off");
        			$(".myboard-show-filter-menuBtn-on").removeClass("myboard-show-filter-menuBtn-on");
        			
        			$("#myboard-show-filter-menuBtn-task").removeClass("myboard-show-filter-menuBtn-off");
        			$("#myboard-show-filter-menuBtn-task").addClass("myboard-show-filter-menuBtn-on");
        			...
        			$("#myboard-content-title1").text("업무");
        			$("#myboard-content-title2").text(filterCnt);
        		})
        		/*일정 클릭*/
        		$("#myboard-show-filter-menuBtn-schedule").click(function() {
        			$(".myboard-show-filter-menuBtn-on").addClass("myboard-show-filter-menuBtn-off");
        			$(".myboard-show-filter-menuBtn-on").removeClass("myboard-show-filter-menuBtn-on");
        			
        			$("#myboard-show-filter-menuBtn-schedule").removeClass("myboard-show-filter-menuBtn-off");
        			$("#myboard-show-filter-menuBtn-schedule").addClass("myboard-show-filter-menuBtn-on");
        			...
        			$("#myboard-content-title1").text("일정");
        			$("#myboard-content-title2").text(filterCnt);
        		})
    		})
    	})
    ...
    <style>
    	* {
    	...
    	#myboard-search-filterBtn-leftBorder {
    		background-color: transparent;
    		border-radius: 0;
    		border: 0;
    	}
    	#myboard-search-filterBtn-leftBorder:before {
    		content: "";
    	...
    	#myboard-content-wrap {
    		height: calc(100% - 20px);
    		margin: 0;
    		padding: 0 30px 0 30px;
    	}
    	#myboard-content-title-wrap {
    		padding: 18px 0 14px;
    		z-index: 1;
    		position: relative;
    	}
    	#myboard-content-title1 {
    		font-weight: 700;
    		font-size: 16px;
    		color: #333;
    	}
    	#myboard-content-title2 {
    		font-size: 17px;
    		color: #999;
    		font-weight: 700;
    	}
    	...
    	.myboard-content-item:first-child {
    		border-top: 0;
    		border-color: transparent;
    	}
    	.myboard-content-item {
    	...
    	.myboard-content-item:hover {
    		border-color: transparent;
    		background: #efebff;
    		cursor: pointer;
    	}
    	.myboard-content-calendar-img {
    		margin-right: 18px;
    		width: 16.88px;
    		height: 18px;
    	...
    	#myboard-search-filter-optionPopup {
    		position: absolute;
    		right: 50%;
    		display: block;
    		top: 40px;
    	...
    	#myboard-search-filter-optionPopup-title {
    		margin-top: 15px;
    		margin-left: 20px;
    		font-size: 13px;
    		font-weight: 700;
    	...
    	.myboard-search-filter-optionPopup-contentLi {
    		display: block;
    		width: 100%;
    		list-style: none;
    	}
    	...
    	#myboard-search-fliter-optionPopup-button-area {
    		displau: flex;
    		justify-content: flex-end;
    		align-items: center;
    		float: right;
    	...
    	#myboard-search-fliter-optionPopup-cancleBtn {
    		height: 36px;
    		padding: 0 32px;
    		line-height: 36px;
    		border: 1px solid #ddd;
    	...
    	#myboard-search-fliter-optionPopup-searchBtn {
    		background: #6449fc;
    		border-color: #6449fc;
    		color: #fff;
    		margin-left: 7px;
    	...
    	.myboard-show-filter-li {
    		margin: 0 0 8px 0;
    		cursor: pointer;
    	}
    	.myboard-show-filter-menuBtn-on, .myboard-show-filter-menuBtn-off {
    		font-size: 13px;
    		color: #555;
    		white-space: nowrap;
    	}
    	.myboard-show-filter-menuBtn-off::before {
    		display: inline-block;
    		content: "";
    	...
    	.myboard-show-filter-menuBtn-on::before {
    		display: inline-block;
    		content: "";
...
<body>
	...
	<div id="rightside" class="fr">
		...
		<div id="myboard-header"><div>내 게시물</div></div>
		<div id="myboard-search-area">
			<div id="myboard-search-inputWrap">
				<img src="images/search-btn.png" id="myboard-search-icon"/>
				<input type="text" id="myboard-search-input" placeholder="검색어를 입력해주세요"/>
				<div id="myboard-search-filterWrap">
					<button id="myboard-search-filterBtn-leftBorder"></button>
					<button id="myboard-search-filterBtn">옵션</button>
				</div>
				<div id="myboard-search-filter-optionPopup" style="display: none;">
					<p id="myboard-search-filter-optionPopup-title">옵션</p>
					<div style="margin: 10px 20px 0 30px;">
						<ul id="myboard-search-filter-optionPopup-contentUl" style="list-style: none;">
							<li class="myboard-search-filter-optionPopup-contentLi">
								<div class="condition-cell title2">프로젝트</div>
								<div class="condition-cell"><input type="text" placeholder="프로젝트명을 입력하세요" id="myboard-search-projectName-input"/></div>
							</li>
							<li class="myboard-search-filter-optionPopup-contentLi">
								<div class="condition-cell title2">작성자</div>
								<div class="condition-cell"><input type="text" placeholder="작성자명 입력(여러명 입력시, 콤마로 구분)" id="myboard-search-writerName-input"/></div>
							</li>
							<li class="myboard-search-filter-optionPopup-contentLi">
						...
						<div id="myboard-search-fliter-optionPopup-button-area">
							<button id="myboard-search-fliter-optionPopup-cancleBtn">취소</button>
							<button id="myboard-search-fliter-optionPopup-searchBtn">검색</button>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div id="myboard-content-wrap">
			<div id="myboard-content-title-wrap">
				<span id="myboard-content-title1">전체</span>
				<span id="myboard-content-title2">${myboardListSize }</span>
				<button id="itemList-filterBtn">필터</button>
				<ul id="myboard-show-filter-ul" style="display: none;">
					<li class="myboard-show-filter-li">
						<div class="myboard-show-filter-menuBtn-on" id="myboard-show-filter-menuBtn-all">전체</div>
					</li>
					<li class="myboard-show-filter-li">
						<div class="myboard-show-filter-menuBtn-off" id="myboard-show-filter-menuBtn-text">글</div>
					</li>
					<li class="myboard-show-filter-li">
						<div class="myboard-show-filter-menuBtn-off" id="myboard-show-filter-menuBtn-task">업무</div>
					</li>
					<li class="myboard-show-filter-li">
						<div class="myboard-show-filter-menuBtn-off" id="myboard-show-filter-menuBtn-schedule">일정</div>
					</li>
				</ul>
			</div>
			<ul id="myboard-content-ul">
				<c:forEach var="dto" items="${myboardList }">
					<li class="myboard-content-item" data-boardIdx="${dto.boardIdx }">
						<div class="itemTypeWrap">
							<c:choose>
						...
						<div class="itemBoardInfoWrap">
							<div><p class="myboard-items-boardName">${dto.boardTitle }</p></div>
							<div><p class="myboard-items-projectName"><span class="project-icon"></span>${dto.projectName }</p></div>
						</div>
						<div class="itemWriteInfo">

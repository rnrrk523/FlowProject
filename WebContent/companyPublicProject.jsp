...
<head>
    ...
    <script>
    	$(function() {
    		...
    		$(document).on("click", ".category-item", function() {
    			let categoryName = $(this).children(".public-item").text();
    			if(categoryName == "전체") {
    				$(".list-items-li").css("display", "list-item");
    				$("#items-category").text(categoryName);
    				$("#items-cnt").text($(".list-items-li:visible").length);
    			}else {
    			...
    			$(".public-item").removeClass("active");
				$(this).children().addClass("active");
				
				if($(".list-items-li:visible").length == 0) {
					$("#notthing-img-1").css("display", "flex");
				}else {
					$("#notthing-img-1").css("display", "none");
				}
    		})
    		
    		/*item 클릭 시*/
    		$(document).on("click", ".list-items-div", function() {
    			let projectIdx = $(this).parent().data("projectidx");
    			if($(this).children(".list-items-participant-state").css("display") == "none") {
	    			if(confirm("참여하시겠습니까?")) {
	    			    alert("참여되었습니다.");
	    			    alert("projectIdx : " + $(this).parent().data("projectidx"));
	    			}
    			}else {
    				alert("projectIdx : " + $(this).parent().data("projectidx"));
    				let pno = $(this).parent().data("projectidx");
    				$.ajax ({
    	    			type: 'post',
    	    	        url: 'MoveprojectAjax',
    	    	        data: {
    		...
    		$("#search-input").keydown(function(event) {
    		    if (event.which === 13) {
    		        ...
    		        $.ajax({
    		        	type: 'post',
    		        	url: 'PublicProjectSearchAjaxServlet',
    		        	data: {
    		        		"memberIdx":${memberIdx},
    		        		"str":str
    		        	},
    		        	success: function(data) {
    		        		...
    		        		let active = $(".public-item.active").text();
    		        		alert(active);
    		        		if(data.length == 0) {
    		        			$("#notthing-img-1").css("display", "flex");
    		        		}else {
	    		        		for(let i=0; i<data.length; i++) {
	    		        			let content = '';
            		        		if(active == data[i].categoryName || active == "전체") {
            		        			content +='<li class="list-items-li" data-projectIdx="'+data[i].projectIdx+'" data-categoryName="'+data[i].categoryName+'">';
            		        		}else {
            		        			content +='<li class="list-items-li" style="display:none;" data-projectIdx="'+data[i].projectIdx+'" data-categoryName="'+data[i].categoryName+'">';
            		        		}
            		        				content += '<div class="list-items-div">'+
        												'<span class="list-items-title">'+data[i].projectName+'</span>'+
        												'<div class="list-items-memberIcon-div">'+
        													'<img class="list-items-memberIcon" src="https://flow.team/flow-renewal/assets/images/icons/icon-post-worker.svg?v=561893fde056ae1ed669a41848c3d08bba938df0"/>'+
        												'</div>'+
        												'<span class="list-items-memberCnt">'+data[i].memberCnt+'</span>'+
        												'<span class="list-items-manager">관리자</span>'+
        												'<span class="list-items-manager-name">'+data[i].firstAdminName+'</span>';
        												if(data[i].participant_yn == 'Y') {
        													content += '<span class="list-items-participant-state" style="display: inline-block;">참여중</span>';
        												}else if(data[i].participant_yn == 'N') {
        													content += '<span class="list-items-participant-state" style="display: none;">참여중</span>';
        												}
        									content += '</div>'+
        										'</li>';
            		        		$("#selction-list-ul").append(content);
	    		        		}
    		        		}
    		        		let cnt = $(".list-items-li:visible").length;
    		        		$("#items-cnt").text(cnt);
    		        	},
    		        	error: function(r, s, e) {
    		        		console.log(r.status);
    ...
    <style>
    	* {
    	...
    	.list-items-memberIcon {
    		width: 13px;
    	}
    	.list-items-memberCnt {
    		margin-right: 6px;
    		color: #999;
    		vertical-align: middle;
    		line-height: 1em;
    	...
    	#notthing-img-3 {
    		width: 160px;
    		height: 116px;
    		background: url('https://team-0aazcm.flow.team/flow-renewal/assets/images/none_data.png?v=72c3c6587d001e80ec3f7681602c90ed8f314052') no-repeat;
    		margin: 0 auto;
    	}
    	#notthing-img-span {
    		display: block;
...
<body>
	...
	<div id="rightside" class="fr">
		...
		<div id="mainContent">
			<div id="search-area">
				<div id="search-wrap">
					<img src="images/search-btn.png" id="search-img"/>
					<input type="text" id="search-input" placeholder="검색어를 입력해주세요"/>
				</div>
			</div>
			<div id="main-sub-header">
				<div id="public-project-header">
					<div id="public-list-area">
						<ul id="category-ul">
							<li class="category-item"><button type="button" class="public-item active">전체</button></li>
							<c:forEach var="str" items="${categoryList }">
								<li class="category-item"><button type="button" class="public-item">${str }</button></li>
							</c:forEach>
						</ul>
					</div>
				</div>
				<div id="selection-wrap">
					<div id="selection-header">
						<span id="items-category">전체</span>
						<span id="items-cnt">${publicProjectListSize }</span>
					</div>
					<ul id="selction-list-ul">
						<c:choose>
							<c:when test="${publicProjectListSize != 0 }">
								<c:forEach var="dto" items="${publicProjectList }">
									<li class="list-items-li" data-projectidx="${dto.projectIdx }" data-categoryname="${dto.categoryName }">
										<div class="list-items-div">
											<span class="list-items-title">${dto.projectName }</span>
											<div class="list-items-memberIcon-div">
												<img class="list-items-memberIcon" src="https://flow.team/flow-renewal/assets/images/icons/icon-post-worker.svg?v=561893fde056ae1ed669a41848c3d08bba938df0"/>
											</div>
											<span class="list-items-memberCnt">${dto.memberCnt }</span>
											<span class="list-items-manager">관리자</span>
											<span class="list-items-manager-name">${dto.firstAdminName }</span>
											<c:choose>
												<c:when test="${dto.participant_yn == 'Y' }">
													<span class="list-items-participant-state" style="display: inline-block;">참여중</span>
												</c:when>
												<c:when test="${dto.participant_yn == 'N' }">
													<span class="list-items-participant-state" style="display: none;">참여중</span>
												</c:when>
											</c:choose>
										</div>
									</li>
								</c:forEach>
								<div id="notthing-img-1" style="display: none;">
									<div id="notthing-img-2">
										<div id="notthing-img-3"></div>
										<span id="notthing-img-span">설정된 프로젝트가 없습니다.<br/>회사 직원 누구나 참여 가능한 프로젝트로 설정해 보세요!</span>
									</div>
								</div>
							</c:when>
							<c:when test="${publicProjectListSize == 0 }">
								<div id="notthing-img-1">
									<div id="notthing-img-2">
										<div id="notthing-img-3"></div>
										<span id="notthing-img-span">설정된 프로젝트가 없습니다.<br/>회사 직원 누구나 참여 가능한 프로젝트로 설정해 보세요!</span>
									</div>

...
public class AdminPage12Action implements Action{
	@Override
...
//		int memberIdx = Integer.parseInt(request.getParameter("memberIdx"));
		...
		ArrayList<UseRecordDto> useRecordList = null;
		try {
			useRecordList = urDao.getUseRecordSearch(companyIdx, "이름", "", startDate.substring(0, startDate.length()-3), endDate.substring(0, endDate.length()-3));
		} catch (Exception e) {
		...
		request.setAttribute("sixMonthsAgoFormatted", sixMonthsAgoFormatted);
		request.setAttribute("useRecordList", useRecordList);
		request.setAttribute("startDate", startDate);
		request.setAttribute("endDate", endDate);
		
		request.getRequestDispatcher("flow_admin12.jsp").forward(request, response);
	}
}
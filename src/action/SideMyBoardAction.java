...
public class SideMyBoardAction implements Action{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<MyBoardSideTabDto> myboardList = new ArrayList<MyBoardSideTabDto>();
		try {
			myboardList = mDao.getSidetabMyBoardOpen(memberIdx);
		} catch (Exception e) {
		...
		request.setAttribute("myboardListSize", myboardList.size());
		
		// 멤버정보
		MemberDao mDao2 = new MemberDao();
		OnlyMemberDto memberInfo = null;
		try {
			memberInfo = mDao2.getMemberInfoAll(memberIdx);
		} catch (Exception e) {
		...
		request.getRequestDispatcher("flow_sideMyBoard.jsp").forward(request, response);
	}
}
...
public class AdminPage14Action implements Action{
	@Override
...
//		int memberIdx = Integer.parseInt(request.getParameter("memberIdx"));
		...
		ArrayList<AdminChangeHistoryDto> adminChangeHistoryList = null;
		try {
			adminChangeHistoryList = achDao.getAdminChangeHistory(companyIdx, "변경자", "", startDate.substring(0, startDate.length()-3), endDate.substring(0, endDate.length()-3));
		} catch (Exception e) {
		...
		request.setAttribute("adminChangeHistoryList", adminChangeHistoryList);
		
		request.getRequestDispatcher("flow_admin14.jsp").forward(request, response);
	}
}
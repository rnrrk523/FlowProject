...
public class TaskAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			Mlist1 = dao.GetProjectMemberProfile(ProjectIdx, 1, 1);
		} catch (Exception e) {
			e.printStackTrace();
		}
		ArrayList<MemberDto> Mlist2 = null;
		try {
			Mlist2 = dao.GetProjectMemberProfile(ProjectIdx, 2, 1);
		} catch (Exception e) {
			e.printStackTrace();
		}
		ArrayList<MemberDto> Mlist3 = null;
		try {
			Mlist3 = dao.GetProjectMemberProfile(ProjectIdx, 3, 1);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		TaskViewOptionDto TVdto = null;
		try {
			TVdto = tdao.SelectTaskViewOption(idx);
		} catch (Exception e) {
			e.printStackTrace();
		}

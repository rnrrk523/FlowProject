...
@WebServlet("/loginRecordSearchMonthAjaxServlet")
public class loginRecordSearchMonthAjaxServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String endOfMonthFormatted = dateFormat.format(endOfMonth);
		String startDate = endOfMonthFormatted.replace(endOfMonthFormatted.substring(8, 10), "01");
		LoginRecordDao lrDao = new LoginRecordDao();
		ArrayList<LoginRecordDto> searchList = new ArrayList<LoginRecordDto>();
		try {
			searchList = lrDao.getLoginRecordSearchDto(companyIdx, standard, str, startDate, endOfMonthFormatted);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}

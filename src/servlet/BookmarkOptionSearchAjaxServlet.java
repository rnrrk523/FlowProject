...
@WebServlet("/BookmarkOptionSearchAjaxServlet")
public class BookmarkOptionSearchAjaxServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int memberIdx = Integer.parseInt(request.getParameter("memberIdx"));
		String projectSearchName = request.getParameter("projectSearchName");
		String writerSearchName = request.getParameter("writerSearchName");
		String selectSearchDate = request.getParameter("selectSearchDate");
		...
		try {
			bookmarkList = bDao.getBookmarkSideTabDtoList(memberIdx, projectSearchName, writerSearchName, selectSearchDate, selectTarget);
		} catch (Exception e) {

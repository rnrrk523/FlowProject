...
@WebServlet("/ProjectChatAjax")
public class ProjectChatAjax extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int chatIdx = 0;
		try {
			chatIdx = dao.ProjectChattingSearch(projectIdx);
			list = dao.ProjectMemberView(projectIdx);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		if(chatIdx == 0) {
			try {
				chatIdx = dao.ChatRoomIdx();
				dao.ChatCreateProject(title, memberIdx, projectIdx);
				for(ProjectMemberViewDto dto : list) {
					dao.ChatMemberInsert(chatIdx, dto.getMemberIdx(), 'Y');
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		try {
			ChatDto = dao.ChattingRoomContents(chatIdx);
			AdminList = dao.AdminChatMemberList(chatIdx);
			ChatContentsList = dao.ChatContentsList(chatIdx);
		} catch (Exception e) {
			e.printStackTrace();
		}
